...
本目录下的 5 个 YAML 文件组成这条流程。当前 `main` 没有 Ruleset 或 Branch Protection，不要求 Pull Request 或 Required status checks；配置见 [docs/setup.md](../../docs/setup.md)。

**GitHub Actions 结构架构图（Archify，用浏览器打开 HTML，可切换主题/导出 PNG）**

| 文件                                                                          | 说明                                                                        |
| ----------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| [github-actions-structure.html](html/github-actions-structure.html)           | **业务流程**：两个入口 → 同一 Issue → PR 验收 → 发布或重复关闭              |
