...
本目录下的 5 个 yml 文件就是这条流水线上的 5 个环节。Ruleset 与 Required Checks 的服务端配置见 [docs/setup.md](../../docs/setup.md)。

**GitHub Actions 结构架构图（Archify，用浏览器打开 HTML，可切换主题/导出 PNG）**

| 文件 | 说明 |
|------|------|
| [github-actions-structure.html](github-actions-structure.html) | **执行流程**：on → permissions → jobs/steps → outputs 各阶段如何衔接 |
| [github-actions-yaml-structure.html](github-actions-yaml-structure.html) | **YAML 层级**：`.yml` 内 on、permissions、jobs、steps、outputs 的嵌套与引用 |

源 JSON：`github-actions-structure.workflow.json`、`github-actions-yaml-structure.architecture.json`（可用 Archify 重新渲染）。
...
flowchart LR
  ...
  subgraph issue [变成 GitHub Issue]
    C[feedback-intake.yml]
    D[publish-conversation-issues.yml]
  end
  E[issue-delivery.yml]
  F[pr-gate.yml]
  G[pr-outcome.yml]
  H[生产环境]
...
| 阶段 | 对应文件 | 用人话说 |
|------|----------|----------|
| 1. 收需求 | `feedback-intake.yml` 或 `publish-conversation-issues.yml` | 把「用户说的话」整理成一份结构化的 **Issue Contract**，并在 GitHub 上**创建 Issue** |
| 2. 写代码 | `issue-delivery.yml` | 让 Codex **改仓库代码**，推一个分支，**开 Draft PR** |
| 3. 验 PR | `pr-gate.yml` | 跑测试、构建、AI 审查、部署预览环境并做 Smoke Test；在 PR 上显示 4 个**必须通过**的检查 |
...
| **跑不起来时** | AI 认为信息不够 → 不建 Issue，Feedback 留在库里等人补证据 |
...
| **publish** | 创建 Issue、写 Problem、dispatch Delivery | Artifacts `intake-result` + `intake-state`；App Token | GitHub Issue；Supabase 更新；触发 `issue-delivery.yml` |
...
| 创建 Issue 后谁启动 Delivery | publish 里 **dispatch** 显式调用 | GitHub 的 **Issue 被打开** 事件触发 |
...
| 项目 | 说明 |
|------|------|
| **什么时候跑** | 新建 Issue 时（对话来源）；或 Feedback 流程 publish 完成后 **dispatch**（带 issue 编号） |
...
Issue → prepare → build → publish → Draft PR → pr-gate.yml
              ...
              └── proceed=false（Feedback 的 issues.opened 空跑）
```

**proceed**：Feedback 建 Issue 会同时触发 `issues.opened` 与 Intake dispatch；prepare 对 `issues` + `source.kind=feedback` 设 `proceed=false`，只让 dispatch 那次真正 build。详见 [issue-delivery.yml.md § proceed](issue-delivery.yml.md#proceed-逻辑)。

更完整的脚本步骤、validate-diff 双跑原因与 R3 分支见 [issue-delivery.yml.md](issue-delivery.yml.md)。

---

### `pr-gate.yml` — 每次 PR 更新都要过的四道关
...
| **PR 上会看到什么** | 四个 Required Checks：`verify`、`build`、`independent-review`、`preview-smoke` |
...
sequenceDiagram
  ...
  SB->>FI: 定时认领
  FI->>GH: 创建 Issue
  FI->>ID: workflow_dispatch 启动 Delivery

  Codex->>PQ: enqueue Contract
  PQ->>PC: 定时发布
  PC->>GH: 创建 Issue
  GH->>ID: issues.opened 启动 Delivery
```
...
| **Job `name:`** | `preview-smoke` | PR 上的检查名 | 可以；改了要和 Ruleset 里 Required Checks 对齐 |
...
| 文件 | Workflow 名称 | 触发 | Jobs | 暴露 Required Checks |
|------|---------------|------|------|----------------------|
...
| `issue-delivery.yml` | Issue Delivery | Issue 打开 + dispatch | prepare → build/analyze-r3 → publish* | 无 |
...
| [`.ai/policy.yaml`](../../.ai/policy.yaml) | 风险等级、Repair 次数、Required Checks 名称 |
| [`docs/setup.md`](../../docs/setup.md) | Ruleset、Secrets、自托管 Runner 安装 |
