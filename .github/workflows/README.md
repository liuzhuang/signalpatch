...
本目录下的 5 个 YAML 文件组成这条流程。`main` Ruleset 只禁止删除和强制推送，不要求 Pull Request 或 Required status checks；配置见 [docs/setup.md](../../docs/setup.md)。
...
| 阶段 | 对应文件 | 用人话说 |
|------|----------|----------|
| 1. 收需求 | `feedback-intake.yml` 或 `publish-conversation-issues.yml` | 创建 `content:raw` Issue；非重复时在同一个 Issue 上改为 `content:processed`，重复时评论并关闭 |
| 2. 写代码 | `issue-delivery.yml` | 让 Codex **改仓库代码**，推一个分支，**开 Draft PR** |
| 3. 验 PR | `pr-gate.yml` | 跑测试、构建、AI 审查、部署预览环境并做 Smoke Test；检查结果供自动化验收，不作为 Ruleset 强制条件 |
...
| 创建 Issue 后谁启动 Delivery | publish 里 **dispatch** 显式调用 | publish 里 **dispatch** 显式调用 |
...
| 项目 | 说明 |
|------|------|
| **什么时候跑** | Feedback 或 Codex 发布器把 Issue 晋升为 processed 后 **dispatch**（带 Issue 编号） |
...
**proceed**：Delivery 只接受显式 dispatch 和 `content:processed` Issue。raw Issue 不会启动 Builder。
...
| **PR 上会看到什么** | 四个检查：`verify`、`build`、`independent-review`、`preview-smoke`；不由 Ruleset 强制 |
...
sequenceDiagram
  ...
  PQ->>PC: 定时发布
  PC->>GH: 创建 Issue
  PC->>ID: workflow_dispatch 启动 Delivery
```
...
| `issue-delivery.yml` | Issue Delivery | dispatch | prepare → build/analyze-r3 → publish* | 无 |
