...
[pr-outcome.yml](pr-outcome.yml) 在 **PR Gate**（workflow 名 **`PR Gate`**，非文件名）**整轮结束**后触发：Gate **全绿**则 **合并 PR、promote 预览部署、跑 Production Smoke、关闭 Issue**；Gate **失败**则先分类：应用失败进入有上限的 Repair，基础设施首次失败重跑 Gate，其余情况转人工。
...
| **failure** | **collect-failure** 后进入 Repair、重跑 Gate 或转人工   | trust 成功即结束（**不** Repair）   |
...
flowchart TD
  ...
  cf -->|configuration / infrastructure exhausted| mhr
  cf -->|evidence missing / collection failed| mhr
  cf -->|application| rep
  rep -->|success| pub
  rep -->|failure| mhr
...
| **collect-failure**     | `ubuntu-latest`               | 失败日志、指纹、三类分类；基础设施可重跑一次   | 非 automation 或 Gate 成功   |
...
| **mark-human-required** | `ubuntu-latest`               | 配置/基础设施终态或 Repair 失败 → Issue 转人工 | 无需人工处理                 |
...
**仅 automation 且 Gate 非 success**：拉失败日志 → **Failure Fingerprint** → **配置 / 基础设施 / 应用** 分类；基础设施且首次 attempt 时 **重跑 Gate**（不修代码）。
...
| `kind`               | 含义                         | 典型匹配                                                            |
| -------------------- | ---------------------------- | ------------------------------------------------------------------- |
| **`configuration`**  | 仓库自动化配置缺失           | `ERR_PNPM_NO_SCRIPT`、pnpm/npm 明确报告脚本不存在                   |
| **`infrastructure`** | Runner/网络/Actions 临时故障 | `runner lost communication`、`econnreset`、`service unavailable` 等 |
| **`application`**    | 测试/代码/审查失败           | 未命中配置或基础设施模式                                            |

独立 Reviewer 拒绝但结构化证据缺失时，Workflow 会把最终分类改为 `evidence-missing`，直接转人工。
...
重跑命令成功后输出 `infrastructure-retrying`，本轮不再转人工；命令失败或重跑后仍失败时保持 `infrastructure`，由 `mark-human-required` 收敛。配置错误也不做 Codex Repair，避免用业务改动修补仓库脚本配置。

#### Job outputs

| Output         | 值                                                                                                  |
| -------------- | --------------------------------------------------------------------------------------------------- |
| `failure_kind` | `configuration` / `infrastructure` / `infrastructure-retrying` / `application` / `evidence-missing` |
| `fingerprint`  | 稳定哈希                                                                                            |
...
遇到配置错误、基础设施一次重试后仍失败、失败证据无法收集，或 Repair Job 失败（预算用尽、重复指纹、策略违规等）时，给 Issue 打 **`ai:human-required`**、评论并写入 Repair Status **`HUMAN_REQUIRED`**，停止自动循环。

#### 条件

仅 automation PR 进入本 Job；满足以下任一条件即执行：

- `collect-failure` Job 自身失败，且没有已经成功排队的 `infrastructure-retrying`；
- `failure_kind` 为 `evidence-missing`、`configuration` 或重试耗尽后的 `infrastructure`；
- `failure_kind` 为 `application` 且 Repair Job 失败。

Job 使用 Controller 调用 `record-run.mjs --stage repair --state HUMAN_REQUIRED`，网页上的 Repair Status 因而与 GitHub Issue 标签保持一致。
...
| ---------------- | --------------------------- | ------------------------------------------------ |
| **`automation`** | **finalize**（合并+发布）   | **collect-failure** → Repair、重跑 Gate 或转人工 |
...
配置失败：**不**进 Repair，按 R2 配置问题转人工。基础设施失败：**不**进 Repair，首次 attempt 时 **`gh run rerun`** 重跑 Gate；重试后仍失败则转人工。
...
| 阶段                            | record-run stage                | 用户可见 Repair Status |
| ------------------------------- | ------------------------------- | ---------------------- |
| Repair push                     | `repair`                        | `REPAIRING`            |
| 配置/基础设施终态或 Repair 停止 | `repair` + `HUMAN_REQUIRED`     | **`HUMAN_REQUIRED`**   |
...
| [classify-failure.mjs](../../scripts/ai/classify-failure.mjs)                   | 配置/基础设施/应用分类  |
| [require-delivery-approval.mjs](../../scripts/ai/require-delivery-approval.mjs) | Builder/Repair 结果门禁 |
