...
| Codex 对话 | [publish-conversation-issues.yml.md](publish-conversation-issues.yml.md) | Issue **`issues.opened`** |
| Intake 建 Issue 副作用 | 同上，但 prepare 设 **`proceed=false`** | 仅 prepare 空跑，真正构建靠 dispatch |

### 触发方式摘要

| 触发 | `github.event_name` | Contract `source.kind` | `proceed` | 谁负责真正构建 |
|------|---------------------|------------------------|-----------|----------------|
| Feedback Intake **dispatch** | `workflow_dispatch` | `feedback` | **`true`** | 本条 dispatch Run |
| Codex 对话建 Issue | `issues` | `codex-conversation` | **`true`** | 本条 `issues.opened` Run |
| Intake 建 Issue 的副作用 | `issues` | `feedback` | **`false`** | **跳过**；等 Intake 的 dispatch Run |

详见 [proceed 逻辑](#proceed-逻辑)。

### Job 总览

| Job | Runner | 用途（一句话） | 何时跳过 |
|-----|--------|----------------|----------|
| **prepare** | `ubuntu-latest` | 从 Issue 提取 Contract、算 `proceed` / `risk_level`、Schema 校验 | 从不跳过 |
...
1. 运行 [`prepare-issue.mjs`](../../scripts/controllers/prepare-issue.mjs) 从 Issue 提取 Contract。
2. 根据触发事件与 `source.kind` 计算 **`proceed`** 与 **`risk_level`**。
...
| **前置条件** | Issue 须带 `ai:*` 自动化标签（如 Intake 创建的 `ai:ready`） |
| **传给 Codex** | 仅 `contract.json` + 最小 `issue.json`（number/title/url），**不传**整段 Issue body |

### proceed 逻辑

`proceed` 是 **prepare Job 的 Job 级输出**，用来决定：**这一轮是否继续跑 build / analyze-r3**。它不是 Contract 字段，而是 Workflow 根据**触发事件**与 **Contract 来源**推断的开关（类比 Intake 的 [`has_feedback`](feedback-intake.yml.md#has_feedback-逻辑)）。

Feedback Intake 创建 Issue 时会同时产生 `issues.opened` 与显式 `workflow_dispatch`。若两条都跑 build，会并发创建两个 AI 分支。prepare 对 **`issues` + `feedback`** 设 `proceed=false`，只让 dispatch 那次继续。
...
- id: context
  ...
  run: |
    ...
    source_kind=$(node -e 'const c=require("./.ai/runs/delivery/contract.json"); process.stdout.write(c.source.kind)')
    proceed=true
    if [[ "$TRIGGER" == "issues" && "$source_kind" == "feedback" ]]; then
      proceed=false
    fi
...
`proceed=false` 时 prepare 仍会完成（提取 Contract、上传 Artifact），但 **build / analyze-r3 / publish 全跳过**——对 Feedback 的 `issues.opened` 空跑是**预期行为**，不是失败。
