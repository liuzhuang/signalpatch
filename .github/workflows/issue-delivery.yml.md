...
自动入口先把同一个 Issue 从 `content:raw` 晋升为 `content:processed`，标签事件统一启动本 Workflow。`workflow_dispatch` 只保留给运维补跑。
...
raw Issue 不会进入 Builder；`prepare-issue.mjs` 仍会拒绝任何缺少 `content:processed` 或 Contract 标记块的标签事件与手动补跑。
...
| [feedback-intake.yml.md](feedback-intake.yml.md)                                         | 上游 Intake、Issue Contract、Problem、processed 标签事件     |
