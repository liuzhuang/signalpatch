...
| `result.status` | 行为 |
|-----------------|------|
| **`NEEDS_EVIDENCE`** | Feedback 标为 `NEEDS_EVIDENCE`；**不**创建 Issue；**不** dispatch Delivery |
| **`SPEC_READY`** | 按 policy **上调** Contract 风险 → 创建 GitHub Issue → 创建 Problem → Feedback 标为 `PROCESSED` → **workflow_dispatch** `issue-delivery.yml`（带 `issue_number`） |

使用 **显式 dispatch** 启动 Delivery，避免 Feedback 建 Issue 时触发的 `issues.opened` 与 dispatch **双跑** issue-delivery。Delivery 侧通过 **`proceed=false`** 跳过 Feedback 来源的 `issues.opened` Run，详见 [issue-delivery.yml.md § proceed](issue-delivery.yml.md#proceed-逻辑)。
...
flowchart TD
  ...
  branch -->|NEEDS_EVIDENCE| ne[PATCH feedback → NEEDS_EVIDENCE]
  ne --> neExit[stdout + exit 0]
  branch -->|SPEC_READY| risk[requiredRisk 上调 contract.riskLevel]
  risk --> issue[POST GitHub Issue]
  issue --> problem[POST problems 行]
...
| 步骤 | API | 说明 |
|------|-----|------|
| 1 | `PATCH /rest/v1/feedback?id=eq.<state.feedbackId>` | `intake_status: "NEEDS_EVIDENCE"`，`processing_started_at: null` |

**不做的事**：不创建 GitHub Issue；不插入 `problems`；不 dispatch `issue-delivery.yml`。

stdout：`Feedback retained as a Problem candidate pending evidence.`，**exit 0**。

此处 **Problem candidate** 指：Feedback 仍可能发展成可处理的 Problem，但**尚未**在 `problems` 表插入行、也**未**关联 `problem_id`。详见 [Problem 是什么 § Problem 候选 vs 创建 Problem](#problem-候选vs创建-problem)。

Feedback 留在库中等人补证据；用户仍可用 **Tracking ID** 查 Repair Status（无 Problem 行时一般为 **`RECEIVED`**）。
...
Delivery 阶段**只认** Contract JSON 块，不认 Issue 正文里的自由文字。
...
**为何显式 dispatch**：Issue 创建会触发 `issues.opened` 类事件；若再依赖事件触发 Delivery，可能与 dispatch **重复跑** issue-delivery。publish 只用 dispatch；`issue-delivery` 的 prepare 对 Feedback + `issues.opened` 设 [proceed=false](issue-delivery.yml.md#proceed-逻辑)。
...
| 类型 | 名称 | 内容 |
|------|------|------|
| **GitHub** | 新建 Issue | 标题 `[SignalPatch] …`；正文含 Problem 摘要 + `signalpatch-contract` JSON 块；标签 `ai:ready`、`risk:r0` 等 |
| **Supabase** | `problems` 行 | `issue_number`、`spec_ready: true`、`repair_status: QUALIFYING` |
...
#### Problem 候选 vs 创建 Problem

Intake 里常出现两种说法，层次不同：
...
Feedback 提交
  ...
  → collect + qualify
      NEEDS_EVIDENCE → Feedback.intake_status = NEEDS_EVIDENCE
                       【不】插入 problems 行
                       口语称「保留为 Problem 候选」= 仍是待归类/待补证据的 Feedback
      SPEC_READY     → publish 执行「创建 Problem」（见下）
```

**创建 Problem** 在代码里指：[`intake-publish.mjs`](../../scripts/controllers/intake-publish.mjs) 在 **`SPEC_READY`** 路径下，GitHub Issue **创建成功之后**，向 Supabase **插入一行** `problems`，并把原 Feedback **挂到** 这行上。
...
- Intake 判定 **`NEEDS_EVIDENCE`**（无 Issue、无 Contract、无 `problems` 行）
- qualify 失败或 publish 在 Issue 创建前失败
...
flowchart TD
  ...
  PR[(problems 行)]
  FB --> Q
  Q -->|NEEDS_EVIDENCE| FB2[Feedback 仍无 problem_id]
  Q -->|SPEC_READY| ISS
  ISS --> PR
...
| **何时不产生** | **`NEEDS_EVIDENCE`** 时只有判定结果，**没有** Contract，也**不**创建 Issue |
...
Feedback（用户原话 + 脱敏 Context）
  → Intake qualify：证据够不够？
      NEEDS_EVIDENCE → 留在 Supabase，不建 Issue
...
Issue 还会打标签 **`ai:ready`** 和 **`risk:r0`**（等级来自上调后的 `riskLevel`）。
...
只有 `has_feedback=true` 且 qualify 产出 `SPEC_READY` 时，publish 才会创建 Issue 并 dispatch Delivery。
