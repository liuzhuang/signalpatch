...
在 Supabase Dashboard 打开 `Project Settings -> API -> Exposed schemas`，加入 `signalpatch`。Migration 已显式授予最小 schema、表和函数权限；仅修改 Exposed schemas 不足以替代这些授权。
...
- `PRODUCTION_URL`：带 `https://` 的稳定生产地址，GitHub Repository Variable。
