...
CI 基础设施错误先原样重试一次；仍失败才进入 Repair。自动 Repair 最多三次。同一 Fingerprint 连续出现两次、没有有效修改、超出允许路径、风险升级到 R2/R3、修改受保护文件、弱化测试、发现安全或数据风险、验收冲突或三次仍失败时，立即转为 `ai:human-required`。
