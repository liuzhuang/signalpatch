...
Skill 和 `AGENTS.md` 负责指导模型如何工作，但不作为安全边界。允许修改路径、风险等级、最大修复次数、自动合并和生产发布权限，由 `.ai/policy.yaml`、`scripts/ai/`、GitHub Actions 与 GitHub Rulesets 确定性执行。PR Gate 检查用于自动化验收，不作为 `main` Ruleset 的 Required status checks。
