...
本文的现行流程说明已更新到 2026 年 7 月 15 日。历史 Run 表仍保留 2026 年 7 月 13 日当时的配置和故障证据，并明确标注为历史记录。当前 `PR Gate` 只响应 `opened`、`synchronize` 和 `reopened`，不会因 Draft PR 转为 Ready 再触发一轮 Gate。
...
一次性 bootstrap 期间使用了本地忽略目录 `.ai/runs/bootstrap/` 下的确定性 Controller。该 Controller 负责创建 Issue、发布补丁、触发重跑和关闭 Issue；Codex 只负责生成或检查本地文件。日常操作不依赖这个临时 Controller，应使用仓库内 6 个 GitHub Workflow。
...
本文所说的「触发了哪些 Actions」主要指 Workflow Run。下表是 2026 年 7 月 13 日旧配置的 4 个 Workflow 和当时 Run 数；当前仓库另有 `manual-issue-intake.yml` 和 `publish-conversation-issues.yml`，共 6 个 Workflow。
...
| `failure-evidence` | 失败日志、分类结果，以及包含 Reviewer findings 和组合 Fingerprint 的 `repair-evidence.json` |     7 天 |
...
| Job 类型                                                                                         | Runner                           | 凭据边界                                                                        |
| ------------------------------------------------------------------------------------------------ | -------------------------------- | ------------------------------------------------------------------------------- |
| Intake `qualify`、Delivery `build`、R3 `analyze-r3`、Gate `independent-review`、Outcome `repair` | 当前用户的自托管 macOS Runner 池 | Codex 只接收清理后的 `HOME` 和 `PATH`；不接收 GitHub、Supabase 或 Vercel 写凭据 |
...
#### 已解决：`mark-human-required` 仓库作用域

历史 Run `29245899651`、`29249996658` 和 `29251456534` 显示：`mark-human-required` Job 没有 Checkout，调用未带 `--repo` 的 `gh issue edit` 时出现：

```text
failed to run git: fatal: not a git repository
```

PR #32 已为两条 `gh issue` 命令显式增加 `--repo "$GITHUB_REPOSITORY"`。Reviewer 拒绝但结构化 Artifact 缺失时，`collect-failure` 还会输出 `evidence-missing`，并由 `mark-human-required` 把 Issue 转入人工处理，不再停留在 `ai:verifying`。
...
| GitHub Actions Workflow     | 定义 Feedback、手工 Issue、Codex 对话三个入口，以及 Issue Delivery、PR Gate 和 PR Outcome，共六条流程          |
| Event triggers              | 使用 `schedule`、`issues`、`issue_comment`、`workflow_dispatch`、`pull_request` 和 `workflow_run` 触发不同阶段 |
...
Runner 池由 3 个独立安装目录组成。这些目录不是 SignalPatch 源码目录，也不是 Vercel 部署目录。

| Runner 名称           | 安装目录                                        | 工作目录                                              |
| --------------------- | ----------------------------------------------- | ----------------------------------------------------- |
| `SignalPatch-Local`   | `/Users/liuzhuang/actions-runner-signalpatch`   | `/Users/liuzhuang/actions-runner-signalpatch/_work`   |
| `SignalPatch-Local-2` | `/Users/liuzhuang/actions-runner-signalpatch-2` | `/Users/liuzhuang/actions-runner-signalpatch-2/_work` |
| `SignalPatch-Local-3` | `/Users/liuzhuang/actions-runner-signalpatch-3` | `/Users/liuzhuang/actions-runner-signalpatch-3/_work` |

3 个实例均使用 Runner 版本 `2.335.1`，关联 `https://github.com/liuzhuang/signalpatch`，并带有 `self-hosted`、`macOS`、`ARM64` 和 `signalpatch` Label。每个实例一次领取 1 个 Job，因此不同 Issue 或 PR 最多可以并行执行 3 个 Codex Job。

本次按已确认决定暂不创建 `signalpatch-runner` 系统用户，因此 3 个 Runner 使用当前用户运行并共享同一个 HOME。这样可以直接使用已登录的 Codex CLI，但隔离程度低于专用系统用户；不要在这些目录保存 GitHub App、Supabase 或 Vercel 凭据。
...
`svc.sh install` 会分别创建：

```text
/Users/liuzhuang/Library/LaunchAgents/actions.runner.liuzhuang-signalpatch.SignalPatch-Local.plist
/Users/liuzhuang/Library/LaunchAgents/actions.runner.liuzhuang-signalpatch.SignalPatch-Local-2.plist
/Users/liuzhuang/Library/LaunchAgents/actions.runner.liuzhuang-signalpatch.SignalPatch-Local-3.plist
```
...
3. 当前 Runner 池需要读取当前用户 HOME 中的 Codex 登录状态，使用同一用户的 LaunchAgent 最符合本次临时配置。
4. 用终端执行 `./run.sh` 的前台进程会随终端关闭而退出，不适合持续接收 Job。

当前配置仍有以下限制：

- 这些都是用户级 LaunchAgent。Mac 重启后，必须登录 `liuzhuang`，Runner 才会启动；它们不是未登录状态下运行的系统级 Daemon。
...
2026 年 7 月 15 日核对时，`SignalPatch-Local`、`SignalPatch-Local-2` 和 `SignalPatch-Local-3` 均为 Online、非 Busy，并具有相同的 4 个 Label。
...
本节记录本次会话中实际发布的方案变更，以及手工 GitHub Issue、Codex 对话和生产网页 Feedback 三种入口的完整 E2E。所有 Run、PR、Deployment 和页面结果均已在远端或生产环境核对。

### 1. 执行计划及其变化

| 初始安排                         | 调整后的方案                                                             | 调整原因                                               |
| -------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------ |
| Manual Intake 每 5 分钟扫描      | 监听 Issue 创建、正文修改、重新打开、`content:raw` 标签和用户评论事件    | GitHub `schedule` 可能延迟，且不适合等待评论补充上下文 |
| 只读取 Issue 正文                | 读取正文及全部非机器人评论                                               | 评论会继续补充 Problem 的有效证据                      |
| 信息不完整时默认要求补充上下文   | 只拦截非产品意见、与产品无关或没有可识别产品结果的输入                   | 具体实现、断点和 Validator 可以根据仓库现状补齐        |
| 只验证一条 Issue Delivery        | 分别验证手工 Issue、Codex 创建 Issue 和网页 Feedback 三种入口            | 需要确认入口差异不会破坏统一 Delivery 和发布流程       |
| 继续场景 2，不修改恢复流程       | 先修复 Reviewer Evidence、失败指纹和 `mark-human-required`，再恢复场景 2 | 真实 Gate 发现 Repair 只看到 Shell 退出信息            |
| 单个自托管 Runner 串行执行 Codex | 注册 3 个独立 Runner；不同 Issue 或 PR 最多并行运行 3 个 Codex Job       | 单个 Runner 一次只能领取 1 个 Job                      |

最终执行顺序为：先发布事件驱动 Manual Intake，再完成场景 1；场景 2 暴露恢复问题后暂停业务流程，发布 PR #32；场景 2 恢复并完成后，再通过生产网页提交场景 3。三个场景全部完成后才判定本次计划结束。

### 2. 已发布的流程变更

#### 事件驱动 Manual Intake

- PR：[#28](https://github.com/liuzhuang/signalpatch/pull/28)
- 合并提交：`05c7e6dcb3c1c6ef6d919c6247052c5ac3be91e4`
- Manual Intake 不再使用 `schedule`；`workflow_dispatch` 只保留为运维补跑入口。
- Intake 读取触发 Issue 的正文和全部非机器人评论，并计算上下文指纹。
- App Bot 使用专用评论保存经过 Schema 校验的 Contract；机器人产生的标签和评论事件直接跳过。
- 用户在 `ai:ready` 阶段继续评论时生成新修订；Delivery 只接受与实时上下文指纹一致的修订。
- 稀疏产品意见可以由 Intake 结合仓库补齐，不再要求提交者预先指定元素范围、CSS 机制、断点或 Validator。

`Feedback Intake` 仍保留每 5 分钟扫描和 `workflow_dispatch`。它读取 Supabase，而不是 GitHub Issue，因此不属于本次移除的 Manual Intake 定时器。

#### PR Outcome 恢复能力

- PR：[#32](https://github.com/liuzhuang/signalpatch/pull/32)
- 合并提交：`3ca56e223e60e1c08cfe7f73791c7c23e6a240dd`
- Reviewer 即使返回 `REQUEST_CHANGES`，也会上传结构化 Artifact。
- `collect-failure` 把有界失败摘要和 Reviewer findings 合成为 `repair-evidence.json`。
- Failure Fingerprint 只使用规范化并排序后的 `decision + findings`，避免无关摘要变化绕过重复失败保护。
- Reviewer 拒绝但 Artifact 缺失时输出 `evidence-missing`，并转入 `ai:human-required`。
- `.pnpm-store/` 不再被误判为业务 Diff；Issue 命令显式指定当前仓库。
- 本地验证通过 `pnpm verify`、63 个测试、`pnpm build`、Workflow 校验和 Actionlint。

#### Codex 并发

GitHub API 最终确认以下 3 个 Runner 同时在线：

- `SignalPatch-Local`
- `SignalPatch-Local-2`
- `SignalPatch-Local-3`

现有 Workflow 已按 Issue 或 PR 隔离正常执行，不需要再增加全局 Codex 锁。Feedback Intake 和 Conversation Publisher 继续保持全局串行；同一 Issue 的 Contract digest、上下文修订和 `ai:ready` 状态会在外部写入前再次校验，Repair 发布使用非强制推送。

### 3. E2E 1：手工创建 GitHub Issue

- Issue：[#27「官网首页改版」](https://github.com/liuzhuang/signalpatch/issues/27)
- 用户补充评论：[Issue comment 4976790941](https://github.com/liuzhuang/signalpatch/issues/27#issuecomment-4976790941)
- Manual Intake：[Run 29388463816](https://github.com/liuzhuang/signalpatch/actions/runs/29388463816)
- Issue Delivery：[Run 29388624548](https://github.com/liuzhuang/signalpatch/actions/runs/29388624548)
- PR：[#29](https://github.com/liuzhuang/signalpatch/pull/29)
- Accepted Head：`6322a76896ceb39ecc9ced4713b67abf6aab2b9e`
- 合并提交：`b1303ff4b443e013a5812f1ecfb954ecc04c393f`
- PR Gate：[Run 29388977058](https://github.com/liuzhuang/signalpatch/actions/runs/29388977058)
- Preview：`https://signalpatch-ov15voz5v-liuzhuangoutlookcoms-projects.vercel.app`
- PR Outcome：[Run 29389111619](https://github.com/liuzhuang/signalpatch/actions/runs/29389111619)
- 最终验收：[Issue comment 4976879366](https://github.com/liuzhuang/signalpatch/issues/27#issuecomment-4976879366)

结果：Issue 已关闭并标记 `ai:done`。Preview 和 Production Smoke Test 均为 7/7。生产页面在 `1280×720` 桌面视口和 `390×844` 移动视口完成居中验收，移动端无水平溢出。

### 4. E2E 2：通过 Codex 创建 Issue

- 本地队列 Request ID：`886ba418-0a87-458f-bf6f-a5f35bc22d5a`
- Issue：[#30](https://github.com/liuzhuang/signalpatch/issues/30)
- Issue Delivery：[Run 29389238028](https://github.com/liuzhuang/signalpatch/actions/runs/29389238028)
- PR：[#31](https://github.com/liuzhuang/signalpatch/pull/31)
- Accepted Head：`cc24364a13eb5699570f1e10ba9fb78733fb04e8`
- 合并提交：`0e94fbe9297c6f9d11b1b791fcf7f84e5de3cd84`
- 最终 PR Gate：[Run 29390427295](https://github.com/liuzhuang/signalpatch/actions/runs/29390427295)
- Preview：`https://signalpatch-grinduj4g-liuzhuangoutlookcoms-projects.vercel.app`
- PR Outcome：[Run 29390592598](https://github.com/liuzhuang/signalpatch/actions/runs/29390592598)
- 最终验收：[Issue comment 4977057084](https://github.com/liuzhuang/signalpatch/issues/30#issuecomment-4977057084)

首次 Gate [29389436640](https://github.com/liuzhuang/signalpatch/actions/runs/29389436640) 因 AC-2 的回归断言不完整而失败。随后 Outcome [29389515537](https://github.com/liuzhuang/signalpatch/actions/runs/29389515537) 暴露 3 个恢复问题：Reviewer Artifact 未上传、`.pnpm-store/v11/index.db` 超出允许路径、`mark-human-required` 缺少仓库上下文。这些问题由 PR #32 修复。

修复合并后，PR #31 更新到最新 `main` 并重新运行 Gate。第二次 Reviewer 返回 `APPROVE`，因此最终流程没有产生新的 Repair Commit；PR #32 验证的是结构化 Evidence 接线、静态回归测试和 R2 Gate，不应把它描述为一次修复后的真实 Repair 成功运行。

结果：Issue 已关闭并标记 `ai:done`。生产 `/about` 显示唯一的「返回首页」链接，`href` 为 `/`；浏览器激活链接后进入首页并显示 `SignalPatch` 一级标题。

### 5. E2E 3：通过生产网页提交 Feedback

提交内容：

> Feedback 提交面板的说明不够明确。请在「提交 Feedback」标题下明确显示：无需登录，提交后会获得 Tracking ID，可随时查询处理状态。文案要简洁并在桌面和移动端保持可见；其他实现细节自行补充，我都同意。

- Tracking ID：`2bc93e60-cec5-4880-ba44-6f3f36acaaf3`
- Evidence 引用：`feedback:2bc93e60-cec5-4880-ba44-6f3f36acaaf3`
- Feedback Intake：[Run 29390873573](https://github.com/liuzhuang/signalpatch/actions/runs/29390873573)
- Intake 结果：`SPEC_READY`、`R1`，没有要求补充上下文
- Issue：[#33](https://github.com/liuzhuang/signalpatch/issues/33)
- Issue Delivery：[Run 29391028516](https://github.com/liuzhuang/signalpatch/actions/runs/29391028516)
- PR：[#34](https://github.com/liuzhuang/signalpatch/pull/34)
- Accepted Head：`d1ee7acc0a1bbf57df5401d7e263412e0898ef7e`
- 合并提交：`4280e196eb8c5238d5948651036f11664078c58f`
- PR Gate：[Run 29391397492](https://github.com/liuzhuang/signalpatch/actions/runs/29391397492)
- Preview：`https://signalpatch-j76dlh158-liuzhuangoutlookcoms-projects.vercel.app`
- PR Outcome：[Run 29391554200](https://github.com/liuzhuang/signalpatch/actions/runs/29391554200)
- 最终验收：[Issue comment 4977182873](https://github.com/liuzhuang/signalpatch/issues/33#issuecomment-4977182873)

状态实际经历 `RECEIVED → QUALIFYING → BUILDING → VERIFYING → RELEASED`。Preview 和 Production Smoke Test 均为 9/9。生产页面在桌面和移动视口都显示精确文案「无需登录，提交后会获得 Tracking ID，可随时查询处理状态。」；移动视口的 `scrollWidth` 和 `clientWidth` 均为 `390`。页面状态查询表单使用本次 Tracking ID 后显示 `RELEASED`。

### 6. 重复执行核对

- Issue #27 的标签和 Bot 评论事件没有启动第二个有效 Delivery。
- Issue #30 的额外 Delivery Run `29389261666`、`29389415175` 均为 `skipped`；相关 Manual Intake Run 也全部跳过。
- Issue #33 只有 Delivery Run `29391028516` 获得执行权；同批次的 `29391028548`、`29391028427`、`29391028352` 均为 `skipped`。
- Issue #33 从创建、状态标签变化到关闭产生的额外 Manual Intake Run 均为 `skipped`。

这些结果说明，同一 Issue 的事件可能产生多个 GitHub Workflow Run，但只有满足实时标签、Contract digest 和上下文修订条件的 Run 会继续执行外部写入。

### 7. 最终状态与边界

- Issue #27、#30 和 #33 均已关闭并标记 `ai:done`。
- 2026 年 7 月 15 日最终 Production 为 `https://signalpatch.meact.dev`。
...
- [ADR 0019：分层 Workflow 编排](./adr/0019-four-workflow-orchestration.md)
