...
flowchart TD
  ...
  F --> G[显式 dispatch Issue Delivery]
...
| `Issue Delivery`              | `workflow_dispatch`               | 只读取 processed Issue Contract，运行 Builder 或 R3 分析，创建 Draft PR |
...
3. `publish`：使用 GitHub App Token 和 Supabase Service Role；先创建 `content:raw` Issue，`SPEC_READY` 且未重复时在同一个 Issue 上改为 `content:processed`，写入 Problem 和 `Issue Contract`，然后显式 dispatch `Issue Delivery`。
...
- `workflow_dispatch`：Feedback Intake 或 Codex 发布器把同一个 Issue 晋升为 `content:processed` 后，显式传入 `issue_number`。
- 并发组按 Issue number 隔离，同一个 Issue 不并行执行两次 Delivery。

Issue Delivery 不监听 `issues.opened`，也不接收 `content:raw` Issue。两个入口在发布阶段完成统一去重和 raw → processed 晋升后，才显式启动一次 Delivery，从触发源消除重复创建 PR 的竞态。
...
| Event triggers              | 使用 `schedule`、`workflow_dispatch`、`pull_request` 和 `workflow_run` 触发不同阶段               |
