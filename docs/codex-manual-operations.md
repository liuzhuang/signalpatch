...
本文以 2026 年 7 月 13 日合并后的 `main` 为准。当前 `PR Gate` 只响应 `opened`、`synchronize` 和 `reopened`，不会因 Draft PR 转为 Ready 再触发一轮 Gate。
...
完成 GitHub App、Repository Variables、Repository Secrets、Labels、`production` 与 `r2-approval` Environment、`main` Ruleset 和自托管 Runner 配置。Ruleset 要求以下 4 个 Required Checks：
...
`trust` 也是 PR Gate Job，但不是 Ruleset 中的业务验收 Check；它负责在读取敏感权限前确认 PR 来源、作者、分支和 Head SHA。
...
一次性 bootstrap 期间使用了本地忽略目录 `.ai/runs/bootstrap/` 下的确定性 Controller。该 Controller 负责创建 Issue、发布补丁、触发重跑和关闭 Issue；Codex 只负责生成或检查本地文件。日常操作不依赖这个临时 Controller，应使用仓库内 4 个 GitHub Workflow。
...
flowchart TD
  ...
  D -->|NEEDS_EVIDENCE| E[保留 Problem 候选]
  D -->|SPEC_READY| F[Controller 创建 Issue Contract]
  F --> G[Issue Delivery prepare]
...
| Workflow          | 触发方式                               | 主要职责                                                                  |
| ----------------- | -------------------------------------- | ------------------------------------------------------------------------- |
| `Feedback Intake` | 每 5 分钟或 `workflow_dispatch`        | 领取一个待处理 Feedback，补齐 Feedback Context，决定是否达到 `SPEC_READY` |
| `Issue Delivery`  | `issues.opened` 或 `workflow_dispatch` | 读取 Issue Contract，运行 Builder 或 R3 分析，创建 Draft PR               |
...
| `R1` | 局部应用逻辑或兼容性缺陷                            | Required Checks 通过后自动合并和发布     |
...
`pnpm verify` 固定包含 Prettier Check、ESLint、TypeScript Type Check、Vitest、SQL 校验和 Workflow 校验。失败时先读取真实日志，不要通过放宽测试、策略或 Required Checks 解决。
...
### 5. GitHub App、Labels、Environment 和 Ruleset
...
`main` Ruleset 至少要求：禁止删除和强制推送、必须通过 PR、必须通过 `verify`、`build`、`independent-review`、`preview-smoke`。不使用绕过 Ruleset 的个人 Token。
...
3. `publish`：使用 GitHub App Token 和 Supabase Service Role；`SPEC_READY` 时创建 Issue、Problem 和 `Issue Contract`，然后显式 dispatch `Issue Delivery`。

如果结果为 `NEEDS_EVIDENCE`，Feedback 会回到该状态，不创建 Issue，也不修改代码。不要为了让流程继续而伪造证据。
...
应用 Feedback 发布成功后，Controller 会自动 dispatch；需要重试时，在 UI 选择 `Issue Delivery` 并输入 Issue number，或执行：
...
## 六、PR Gate：trust 与四项 Required Checks
...
PR Gate 共包含 5 个 Job。`trust` 是安全前置检查；其余 4 个是 `main` Ruleset 的 Required Checks。
...
Required Checks 全部通过后，`PR Outcome` 使用 `production` Environment，自动合并 PR、提升已验收 Deployment、执行 Production Smoke Test，并在成功后关闭 Issue。

### 2. R2

Required Checks 全部通过后，`PR Outcome` 会停在 `r2-approval`：
...
`finalize` 仍会在合并前执行 `gh pr ready`，但当前 `PR Gate` 不再监听 `ready_for_review`。同一个 Head SHA 不应因为 Draft 转为 Ready 再生成一轮 Required Checks。
...
3. 不绕过 Ruleset，也不手动降低 Required Checks。
...
| Required Checks | PR 的 `verify`、`build`、`independent-review`、`preview-smoke`              |
...
[ ] GitHub App、Labels、Environment、Ruleset 和 Runner 状态正常
...
[ ] PR Gate 四项 Required Checks 全部通过
...
本文所说的「触发了哪些 Actions」主要指 Workflow Run。仓库只定义 4 个顶层 Workflow：
...
| `publish` | `ubuntu-latest` | 否              | 创建短时 GitHub App Token；发布 `SPEC_READY` Issue 或保存 `NEEDS_EVIDENCE`；更新 Supabase 并显式触发 Issue Delivery |
...
- `issues.opened`：普通 Issue 打开时自动检查 Issue Contract。
- `workflow_dispatch`：应用 Feedback 的 `publish` 阶段或人工重试时显式传入 `issue_number`。
- 并发组按 Issue number 隔离，同一个 Issue 不并行执行两次 Delivery。

应用 Feedback 创建 Issue 时可能同时出现一个 `issues.opened` Run 和一个 `workflow_dispatch` Run。`prepare` 会读取 `source.kind`；当来源是 `feedback` 且事件是 `issues` 时，自动事件只做 Contract 检查并将 `proceed=false`，真正构建由显式 Dispatch Run 执行。这样可以避免重复创建 PR。
...
`main` Ruleset 要求 `verify`、`build`、`independent-review` 和 `preview-smoke`。`trust` 是读取后续权限和证据前的安全检查，但不是 Ruleset 中的 Required Check。
...
| #3    | [`29247139817`](https://github.com/liuzhuang/signalpatch/actions/runs/29247139817) | 失败 | `finalize` 把 Draft 标为 Ready 后触发新 Gate，Ruleset 报告 4 个 Required Checks 仍在等待                 |
...
- Ruleset：保护默认分支，禁止删除和强制推送，并要求 4 个 Required Checks。
...
Feedback Intake 创建 Issue、显式 Dispatch Issue Delivery，以及各阶段写入 Supabase 的操作由 Node.js Controller 直接调用 REST API。因此，`gh` 命令清单不是全部外部 API 调用清单。
...
| GitHub Actions Workflow     | 定义 Feedback Intake、Issue Delivery、PR Gate 和 PR Outcome 四条自动化流程                        |
| Event triggers              | 使用 `schedule`、`workflow_dispatch`、`issues`、`pull_request` 和 `workflow_run` 触发不同阶段     |
...
| Ruleset                     | 保护 `main`，要求 Pull Request、禁止删除和强制推送，并要求 4 个 Check 通过                        |
...
1. **确认 `r2-approval` 已放行**：R2 的 `finalize` Job 绑定 `r2-approval` Environment。Required reviewer 点击 `Approve and deploy` 后，GitHub 才允许该 Job 继续。这个批准只允许当前 Deployment Job 执行，不等于永久批准 PR，也不跳过 Required Checks。
...
SignalPatch 的 `main` Ruleset 要求以下 4 个 Check 全部成功：
...
| Required Check       | Ruleset 明确要求成功的 Check；未成功时不能合并                  |
| `trust` Job          | PR Gate 内的安全前置检查，但当前不是 Ruleset Required Check     |
...
| Actions       | Read and write | 显式 Dispatch Issue Delivery，以及读取或控制允许范围内的 Actions 运行                                            |
...
当前 `main` Ruleset 没有配置 bypass actor。GitHub App 必须像其他调用者一样满足 Pull Request 和 4 个 Required Checks，不能绕过门禁直接写入 `main`。
...
- PR Head、Required Checks 或 Workflow Run 变化后，需要重新核对并可能重新审批。
