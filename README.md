...
- 在用户明确确认后，Codex 也可作为 Issue 上报入口：`gh` 已登录且有 Issue 权限时直接创建，否则把已校验的 `codex-conversation` Issue Contract 写入受控本地队列，由独立发布控制器创建。
...
Contract 必须通过 `.ai/schemas/issue-contract.schema.json`，`source.kind` 为 `codex-conversation`，且 `source.references` 必须含有 `conversation:explicit-user-confirmation`。直接路径复用同一套 raw/processed、去重和 Delivery 流程；`gh` 不可用、未登录或没有 Issue 写权限时，才写入队列。
